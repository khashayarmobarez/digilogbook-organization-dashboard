...
// queries
import { useAClubCourseCounts, useClubAllStudents } from '@/api/useClub';
...
const StudentList = ({params}) => {
    ...
    const { clubId, studentListTypeId } = params;

    // queries
    const { data: courseCountsData, isLoading: courseCountsLoading } = useAClubCourseCounts(clubId);
    // id 1 is for active students and id 2 is for history student
    const { data: AllStudents, isLoading: AllStudentLoading, error: AllStudentError } = useClubAllStudents(studentListTypeId ,clubId);
                    ...
                    <div className='w-full flex flex-col gap-y-4 items-center'>
                        {AllStudents.data.length > 0 && AllStudents.data &&
                            AllStudents.data.map((student) => (
                                <ACourseStudentBox ownerId={clubId} key={student.id} studentData={student} isForHistory={studentListTypeId === '1' ? false : true} isForClub={true} />
                            ))
                        }
                    </div>
